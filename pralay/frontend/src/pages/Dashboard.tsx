...
export default function Dashboard() {
  ...
  const [scenarioSteps, setScenarioSteps] = useState<DemonstrationStep[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const predRes = await axios.get('/api/v1/predictions');
        if (predRes.data && predRes.data.length > 0) {
          setAssessments(predRes.data);
        }
      } catch (err) {
        console.warn('API predictions endpoint unreachable, using local deterministic state.');
      }

      try {
        const scenRes = await axios.post('/api/v1/scenarios/run-12-step?village_id=1');
        if (scenRes.data && scenRes.data.steps) {
          setScenarioSteps(scenRes.data.steps);
        }
      } catch (err) {
        console.warn('API scenario endpoint unreachable, loading built-in 12-step scenario.');
      }
    };
  ...
  const selectedAssessment = assessments.find(a => a.village_id === selectedVillageId) || assessments[0];

  const handleOpenCitizenCard = async () => {
    try {
      const res = await axios.get(`/api/v1/alerts/citizen/${selectedVillageId}`);
      if (res.data) {
      ...
      console.warn('Citizen alert endpoint unreachable, synthesizing from active assessment');
    }
    ...
    const synth: CitizenAlertCard = {
      ...
      estimated_lead_time: selectedAssessment.lead_time.minutes ? `${selectedAssessment.lead_time.minutes} Minutes` : "Advisory Status",
        ...
        if (a.village_id === 1) {
          return {
            ...
            lead_time: {
              ...
              hours: step.assessment.estimated_lead_time_minutes ? Number((step.assessment.estimated_lead_time_minutes / 60).toFixed(2)) : null,
